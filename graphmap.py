...
for v in G.vertices():
    ...
    if i-1 > -1:
        if G.edge(coord2(top, framematrix.shape[1]), v) == None:
            G.add_edge(v, G.vertex(coord2(top, framematrix.shape[1])))
    if i+1 < framematrix.shape[0]:
        if G.edge(coord2(bottom, framematrix.shape[1]), v) == None:
            G.add_edge(v, G.vertex(coord2(bottom, framematrix.shape[1])))
    if j-1 > -1:
        if G.edge(coord2(left, framematrix.shape[1]), v) == None:
            G.add_edge(v, G.vertex(coord2(left, framematrix.shape[1])))
    if j+1 < framematrix.shape[1]:
        if G.edge(coord2(right, framematrix.shape[1]), v) == None:
            G.add_edge(v, G.vertex(coord2(right, framematrix.shape[1])))
